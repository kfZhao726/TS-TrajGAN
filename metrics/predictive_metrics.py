...
from utils import extract_time, MinMaxScaler
...
def predictive_score_metrics(ori_data, generated_data):
    ...
    def predictor(x, t):
        ...
        with tf.variable_scope("predictor", reuse=tf.AUTO_REUSE) as vs:
            ...
            p_vars = [v for v in tf.all_variables() if v.name.startswith(vs.name)]  # vs.name = predictor ?

        return y_hat, p_vars

    y_pred, p_vars = predictor(X, T)
    # Loss for the predictor
    p_loss = tf.losses.absolute_difference(Y, y_pred)  # loss_fun: MAE
    # optimizer
